...
//グリッド　デスクトップ2カラム 以下1カラム
function col2Func( $atts, $content = null ) {
	extract( shortcode_atts( array(
        'class' => '',
    ), $atts ) );
    
    return '<div class="d-1of2 t-all m-all ' . $class . '">' . do_shortcode($content) . '</div>';
}
add_shortcode('col2', 'col2Func');

//グリッド　デスクトップ3カラム 以下1カラム
function col3Func( $atts, $content = null ) {
    return '<div class="d-1of3 t-all m-all">' . do_shortcode($content) . '</div>';
}
...
function voiceFunc( $atts, $content = null ) {
    ...
    return '<div class="voice cf ' . $class . $type. '"><figure class="icon"><img src="' . $icon . '"><figcaption class="name">' . $name . '</figcaption></figure><div class="voicecomment">' . $content . '</div></div>';
}
